...
#STAD 
clin <- GDCquery_clinic("TCGA-STAD", type = "clinical", save.csv = TRUE)
sub_clin <- subset(clin, select = c("submitter_id", "ajcc_pathologic_stage", 
                                    "gender", "age_at_index"))

#Somatic Mutations:
query1 <- GDCquery( project = "TCGA-STAD", 
...
write.csv(mut_data,"STAD_tsg.csv")
...
stad_tsg <- unique(xchr_tsg[c("GeneSymbol")])

#sums of male mutations
stad_tsg$male <- c(sum(xchr_tsg$GeneSymbol == "PHF6" & xchr_tsg$gender == "male"),
...
stad_tsg$female <- c(sum(xchr_tsg$GeneSymbol == "PHF6" & xchr_tsg$gender == "female"),
...
stad_tsg$stad_bias <- stad_tsg$male - stad_tsg$female

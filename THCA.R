...
#STAD 
clin <- GDCquery_clinic("TCGA-THCA", type = "clinical", save.csv = TRUE)
sub_clin <- subset(clin, select = c("submitter_id", "ajcc_pathologic_stage", 
                                    "gender", "age_at_index"))

#Somatic Mutations:
query1 <- GDCquery( project = "TCGA-THCA", 
...
write.csv(mut_data,"THCA_tsg.csv")
...
thca_tsg <- unique(xchr_tsg[c("GeneSymbol")])

#sums of male mutations
thca_tsg$male <- c(sum(xchr_tsg$GeneSymbol == "SRPX" & xchr_tsg$gender == "male"),
...
thca_tsg$female <- c(sum(xchr_tsg$GeneSymbol == "SRPX" & xchr_tsg$gender == "female"),
...
thca_tsg$stad_bias <- thca_tsg$male - thca_tsg$female

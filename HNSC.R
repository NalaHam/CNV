...
#HNSC (has NO CANCER LEVEL DATA OR AGE DATA!!!)

#The following is not productive because no gender data!

clin <- GDCquery_clinic("TCGA-HNSC", type = "clinical", save.csv = TRUE)
...
hnsc_tsg <- unique(xchr_tsg[c("GeneSymbol")])

#sums of male mutations
hnsc_tsg$male <- c(sum(xchr_tsg$GeneSymbol == "DMD" & xchr_tsg$gender == "male") 
                  sum(xchr_tsg$GeneSymbol == "AMER1" & xchr_tsg$gender == "male") 

#sums of female mutations
gbm_tsg$female <- c(sum(xchr_tsg$GeneSymbol == "DMD" & xchr_tsg$gender == "female") 
                    sum(xchr_tsg$GeneSymbol == "AMER1" & xchr_tsg$gender == "female")
                    sum(xchr_tsg$GeneSymbol == "FLNA" & xchr_tsg$gender == "female")
                    sum(xchr_tsg$GeneSymbol == "MIR503" & xchr_tsg$gender == "female")
                    sum(xchr_tsg$GeneSymbol == "RBBP7" & xchr_tsg$gender == "female")
